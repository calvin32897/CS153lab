...
found:
  p->state = EMBRYO;
  p->pid = nextpid++;
  p->priority = 15; //cs153_lab2: initialize process' priority value to 15 (middle priority)
...
exit(int status)
{
  ...
  //saving exit status of terminated process
}
...
int waitpid(int pid, int *status, int options){
  ...
  for(;;){
    ...
    samepid = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->pid != pid) //looking for same pid
        continue;
      samepid = 1;
      if(p->state == ZOMBIE){
        // Found one.
...
wait(int* status)
{
  ...
  for(;;){
    ...
    havekids = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->parent != curproc) //if it is a child process
        continue;
      havekids = 1;
      if(p->state == ZOMBIE){
        // Found one.
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      for(q = ptable.proc; q < &ptable.proc[NPROC]; q++){
        if(q->state == RUNNABLE && temp->priority > q->priority){
	  temp = q;
	}
      }
      //run thru and increase the priority of any process that is not temp(waiting process)
      for(r = ptable.proc; r < &ptable.proc[NPROC]; r++){
        if(r->state == RUNNABLE && r!=temp && r->priority > 0){
      ...
      temp->state = RUNNING;
      if(temp->priority < 63){
        temp->priority = temp->priority + 1; //decrease its priority
      }
    ...
    for(;;) {
        ...
        for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
        {
            ...
            for (tempProc = ptable.proc; tempProc < &ptable.proc[NPROC]; tempProc++)
            {
                ...
                if (tempProc->priority < highestPriority_proc->priority)
